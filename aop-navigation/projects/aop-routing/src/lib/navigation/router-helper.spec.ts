    ...
    beforeEach(() => {
      ...
      describe('#modifyRouteTable', () => {
        ...
        it(`should call addNewRoutePath if getRoutePathObj returns undefined`, () => {
            ...
            RouteHelper.modifyRouteTable(mockRouter, routeTransform);
            expect(RouteHelper.addNewRoutePath).toHaveBeenCalledWith(mockRouter, routeTransform.path,
                 routeTransform.component, routeTransform.canActivateGuards);
        });

        it(`should call changeRouteComponent if getRoutePathObj returns a value and
         routeTranform component property is defined`, () => {
            spyOn(RouteHelper, 'getRoutePathObj').and.returnValue({path: '123', component: 'test-comp', canActivate: ['guard 1']});
            spyOn(RouteHelper, 'changeRouteComponent');
            spyOn(RouteHelper, 'updateCanActivateGuards');
            const routeTransform: RouteTransform = {
                path: 'Test3',
            ...
            RouteHelper.modifyRouteTable(mockRouter, routeTransform);
            expect(RouteHelper.changeRouteComponent).toHaveBeenCalledWith(mockRouter, routeTransform.path, routeTransform.component);
        });
        ...
        routeTranform canActivateGuards property is defined`, () => {
            ...
            RouteHelper.modifyRouteTable(mockRouter, routeTransform);
            expect(RouteHelper.updateCanActivateGuards).toHaveBeenCalledWith(mockRouter, routeTransform.path,
